...
の順番で書いていきます。ベースイメージはできるだけ Docker Hub の Official Image を使いましょう。2 から 4 を何度か試しながら修正していきます。

Docker Official Image の説明や、使用しているフレームワークの公式ドキュメントにある例を参考にしても構いません。ただし、各命令の意味を確認してから使用しましょう。
...
コンテナ側のポートを 8080 以外にして実行し、アクセスした様子をスクリーンショットに撮って講習会用チャンネルに投稿しましょう。
